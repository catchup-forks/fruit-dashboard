<div class="chart-value larger-text">
  {{ Utilities::formatNumber(array_values($widget['data'][$widget['layout']]['currentValue'])[0], $widget['format']) }}
</div> <!-- /.chart-value -->

<div class="chart-diff-data text-center">

  <div class="chart-diff @if($widget['className']::isSuccess($widget['data'][$widget['layout']]['currentDiff'])) text-success @else text-danger @endif">
  @if ($widget['data'][$widget['layout']]['currentDiff'] >= 0)
      <span class="fa fa-arrow-up chart-diff-icon"> </span>
  @else
      <span class="fa fa-arrow-down chart-diff-icon"> </span>
  @endif
    <span class="chart-diff-value larger-text">{{ Utilities::formatNumber(array_values($widget['data'][$widget['layout']]['currentDiff'])[0], $widget['format']) }}</span>
  </div> <!-- /.chart-diff -->
